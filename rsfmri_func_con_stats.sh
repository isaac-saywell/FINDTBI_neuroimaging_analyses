...
echo "Producing a brain mask of spatial maps for each ICA signal component at threshold $thr_rs."
...
find "$start_dir" -maxdepth 1 -mindepth 1 -type d | sort | while read -r dir; do
    ...
    for image in ica_signal_component_*.nii.gz; do
        ...
        comp_num=$(echo "$image" | sed 's/.*rsn_\([0-9]\{4\}\)/\1/') # extract the numerical part of component spatial map

        p_mask_output=bin_$image # set binarised mask output name

        fslmaths "$image" -thr $thr_rs -bin "$p_mask_output" # binarise the spatial map at threshold deemed for significant signal (unsure what thresholds to use currently........)

    done

    echo "Binarised spatial maps at threshold $thr_rs created for subject $(basename "$dir")"
